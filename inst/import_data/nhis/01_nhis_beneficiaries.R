...
##------------------------------------------------------------------------------
get_tab1 <- function(x, type = c("left", "right")) {
  if (type == "left") {
    tmp <- x |> 
  ...
  if (type == "left") {
    ...
    names(tmp) <- c("cty_nm", "medipop_cnt", "emplye_estbmt_cnt", 
                    "emplye_popunr_cnt", "emplye_insurd_cnt", 
                    "emplye_dpndnt_cnt", "cvlsvt_estbmt_cnt", 
                    "cvlsvt_popunr_cnt", "cvlsvt_insurd_cnt")
    tmp <- tmp |> 
    ...
    names(tmp) <- c("cvlsvt_dpndnt_cnt", "locly_insurd_cnt", "locly_hshld_cnt", 
                    "medaid_clss1_popunr_cnt", "medaid_clss1_hshld_cnt", 
                    "medaid_clss1_dpndnt_cnt", "medaid_clss2_popunr_cnt", 
                    "medaid_clss2_hshld_cnt", "medaid_clss2_dpndnt_cnt")
  }
...
## 02.01.01. 좌측 페이지
##------------------------------------------------------------------------------
extract_tables(pdf_stat, pages = 1, col_names = FALSE, guess = FALSE) |> 
  (function(x) x[[1]])() |> 
  filter(row_number() <= 54) |> 
  filter(row_number() >= 6) -> tmp

tab_01_left <- get_tab1(tmp, "left")
...
## 02.01.02. 우측 페이지
##------------------------------------------------------------------------------
extract_tables(pdf_stat, pages = 2, col_names = FALSE, guess = FALSE) |> 
  (function(x) x[[1]])() |> 
  filter(row_number() <= 59) |> 
  filter(row_number() >= 11) -> tmp

tab_01_right <- get_tab1(tmp, "right")
...
## 02.02.01. 좌측 페이지
##------------------------------------------------------------------------------
extract_tables(pdf_stat, pages = 3, col_names = FALSE, guess = FALSE) |> 
  (function(x) x[[1]])() |> 
...
## 02.01.02. 우측 페이지
##------------------------------------------------------------------------------
extract_tables(pdf_stat, pages = 4, col_names = FALSE, guess = FALSE) |> 
  (function(x) x[[1]])() |> 
  filter(row_number() <= 60) |> 
  filter(row_number() >= 11) -> tmp

tab_02_right <- get_tab1(tmp, "right")
...
tab_03_right <- get_tab1(tmp, "right")
...
tab_04_right <- get_tab1(tmp, "right")
...
tab_05_right <- get_tab1(tmp, "right")
...
tab_06_right <- get_tab1(tmp, "right")
