...
admi_seoul <- admi_seoul |> 
  ...
  rename_with(function(x) {stringr::str_replace_all(x, "^AE_", "AREA_")}, starts_with("AE_")) |>  
  rename_with(function(x) {stringr::str_replace_all(x, "^PC_", "PRC_")}, starts_with("PC_")) |>    
  rename_with(function(x) {stringr::str_replace_all(x, "_CO$", "_CNT")}, ends_with("_CO")) |> 
  rename_with(toupper) |> 
  tibble::as_tibble()

# dbWriteTable(con, "TB_SEOUL_APT_ADMI", admi_seoul, overwrite = TRUE)
# dbGetQuery(con, "select * from TB_SEOUL_APT_ADMI limit 5;")
readr::write_csv(admi_seoul, 
                 file = here::here("sftp_repository", "public_seoul", "seoul_apt_.csv"))
...
readr::write_csv(admi_seoul_cty, 
                 file = here::here("sftp_repository", "public_seoul", "seoul_apt_cty.csv"))
