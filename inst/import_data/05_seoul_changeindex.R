################################################################################
## 05_seoul_changeindex.R
## 서울시 상권분석서비스(상권변화지표-행정동)
...
admi_seoul <- admi_seoul |> 
  ...
  select(base_yq, mega_cd, mega_nm, cty_cd, cty_nm, admi_cd, admi_nm, 
         TRDAR_CHNGE_IX:SU_CLS_SALE_MT_AVRG) 
...
admi_seoul <- admi_seoul |> 
  ...
  rename_with(function(x) {stringr::str_replace_all(x, "_AVRG$", "_AVG")}, ends_with("_AVRG")) |>  
  rename_with(function(x) {stringr::str_replace_all(x, "IX", "IDX")}, contains("IX")) |>    
  rename_with(function(x) {stringr::str_replace_all(x, "_CO$", "_CNT")}, ends_with("_CO")) |> 
  rename_with(toupper) |> 
  tibble::as_tibble()

dbWriteTable(con, "TB_SEOUL_CHNGIDX_ADMI", admi_seoul, overwrite = TRUE)
# dbGetQuery(con, "select * from TB_SEOUL_CHNGIDX_ADMI limit 5;")
...
##==============================================================================
admi_seoul |> 
  select(BASE_YQ:CTY_NM, TRDAR_CHNGE_IDX, TRDAR_CHNGE_IDX_NM, OPR_SALE_MT_AVG:SU_CLS_SALE_MT_AVG) |> 
  group_by(BASE_YQ, MEGA_CD, MEGA_NM, CTY_CD, CTY_NM, TRDAR_CHNGE_IDX, TRDAR_CHNGE_IDX_NM) |>
...
##==============================================================================
dbWriteTable(con, "TB_SEOUL_CHNGIDX_CTY", admi_seoul_cty, overwrite = TRUE)
# dbGetQuery(con, "select * from TB_SEOUL_CHNGIDX_CTY limit 5;")
