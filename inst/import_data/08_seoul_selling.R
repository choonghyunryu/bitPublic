...
admi_seoul <- admi_seoul |> 
  ...
  select(base_yq, mega_cd, mega_nm, cty_cd, cty_nm, admi_cd, admi_nm, 
         SVC_INDUTY_CD:AGRDE_60_ABOVE_SELNG_CO) 
...
admi_seoul <- admi_seoul |> 
  ...
  rename_with(function(x) {stringr::str_replace_all(x, "^AGRDE_", "AGEGRP_")}, starts_with("AGRDE_")) |> 
  rename_with(function(x) {stringr::str_replace_all(x, "^ML_", "MALE_")}, starts_with("ML_")) |>   
  rename_with(function(x) {stringr::str_replace_all(x, "^FML_", "FEMALE_")}, starts_with("FML_")) |> 
  rename_with(function(x) {stringr::str_replace_all(x, "^MDWK_", "WKDAY_")}, starts_with("MDWK_")) |> 
  rename_with(function(x) {stringr::str_replace_all(x, "^TUES_", "TUE_")}, starts_with("TUES_")) |> 
  rename_with(function(x) {stringr::str_replace_all(x, "^THUR_", "THU_")}, starts_with("THUR_")) |> 
  rename_with(function(x) {stringr::str_replace_all(x, "INDUTY", "INDSTY")}, contains("INDUTY")) |> 
  rename_with(toupper) |> 
  tibble::as_tibble()

dbWriteTable(con, "TB_SEOUL_SELLING_ADMI", admi_seoul, overwrite = TRUE)
# dbGetQuery(con, "select * from TB_SEOUL_SELLING_ADMI limit 5;")
...
##==============================================================================
admi_seoul |> 
  select(BASE_YQ:CTY_NM, SVC_INDSTY_CD, SVC_INDSTY_CD_NM, THSMON_SELNG_AMT:AGEGRP_60_ABOVE_SELNG_CNT) |> 
  group_by(BASE_YQ, MEGA_CD, MEGA_NM, CTY_CD, CTY_NM, SVC_INDSTY_CD, SVC_INDSTY_CD_NM) |>
...
##==============================================================================
dbWriteTable(con, "TB_SEOUL_SELLING_CTY", admi_seoul_cty, overwrite = TRUE)
# dbGetQuery(con, "select * from TB_SEOUL_SELLING_CTY limit 5;")
