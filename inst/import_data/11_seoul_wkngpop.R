...
service_id <- "VwsmAdstrdWrcPopltnW"

con <- dbConnect(SQLite(), here::here("inst", "dbms", "public.sqlite"))

################################################################################
## 02. 서울시 상권분석서비스(직장인구-행정동)
################################################################################
##==============================================================================
## 02.01. 데이터 읽기
##==============================================================================
...
admi_seoul <- admi_seoul |> 
  ...
  select(base_yq, mega_cd, mega_nm, cty_cd, cty_nm, admi_cd, admi_nm, 
         TOT_WRC_POPLTN_CO:FAG_60_ABOVE_WRC_POPLTN_CO) 
...
admi_seoul <- admi_seoul |> 
  ...
  rename_with(function(x) {stringr::str_replace_all(x, "WRC", "WKPC")}, contains("WRC")) |> 
  rename_with(toupper) |> 
  tibble::as_tibble()

dbWriteTable(con, "TB_SEOUL_WKNGPOP_ADMI", admi_seoul, overwrite = TRUE)
# dbGetQuery(con, "select * from TB_SEOUL_WKNGPOP_ADMI limit 5;")
...
##==============================================================================
admi_seoul |> 
  select(BASE_YQ:CTY_NM, TOT_WKPC_POPLTN_CNT:FAG_60_ABOVE_WKPC_POPLTN_CNT) |> 
...
##==============================================================================
dbWriteTable(con, "TB_SEOUL_WKNGPOP_CTY", admi_seoul_cty, overwrite = TRUE)
# dbGetQuery(con, "select * from TB_SEOUL_WKNGPOP_CTY limit 5;")
